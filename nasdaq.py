...
import ml_finance.locales as Location

from ml_finance.ticker_utils import get_tickers, s_and_p_tickers
from ml_finance.utils import enum_extension_files, force_mkdir, save_json

from googlefinance import getQuotes

s_and_p_names = s_and_p_tickers(get_back='Security')
s_and_p_dict = {ticker:s_and_p_names[s_and_p_symbols.index(ticker)] for ticker in s_and_p_tickers()}
...
def finnhub_metrics(ticker):
    ...
    last_year_timestamp = last_year_today_date.timestamp()
...
def analyze_short_term(number_to_return=10):
...
def enumerate_stocks(tickers, Amount_of_API_Calls=0, Stock_Failure=0, Stocks_Not_Imported=0):
...
def the_daemon():
    tickers = get_tickers(NYSE=True, NASDAQ=True, AMEX=True, S_AND_P=True)
    ...
    if os.path.exists(Location.Home(__file__) + "/stocks/short_term"):
        ...
        os.mkdir(Location.Home(__file__) + "/stocks/short_term")
    if not os.path.exists(Location.Home(__file__) + "/stocks/long_term"):
        os.mkdir(Location.Home(__file__) + "/stocks/long_term")
    Amount_of_API_Calls, Stock_Failure, Stocks_Not_Imported = enumerate_stocks(tickers)
    tickers = get_tickers(NYSE=True, NASDAQ=True, AMEX=True, S_AND_P='Done')
    Amount_of_API_Calls, Stock_Failure, Stocks_Not_Imported = enumerate_stocks(tickers, Amount_of_API_Calls=Amount_of_API_Calls,
                                                                            Stock_Failure=Stock_Failure, Stocks_Not_Imported=Stocks_Not_Imported)
def main():
    daemon = input('(D)aemon')
    if daemon == 'D':
        the_daemon()
    else:
        analyze_short_term()
